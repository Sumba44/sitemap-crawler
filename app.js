...
const c = new Crawler({
  ...
  callback: function (error, res, done) {
    if (error) {
      ...
      logFile.write(
        outputString + " || " + count + "/" + urlList.length + " || " + res.options.uri + "\r\n"
      );
      ...
      // Log the output
      console.log(outputRender+ "\t", chalk.blue(count) + chalk.gray("/") + chalk.blueBright(urlList.length) + "\t", chalk.yellow(res.options.uri));
